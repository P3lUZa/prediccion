...
class CRPred:    
    ...
    def predict(self, indexes = [-1]):
        results = pd.DataFrame()
        for i, indx in enumerate(indexes):
            results.loc[i,'index'] = indx
            p = pd.DataFrame(self.prep[self.feature_names].iloc[indx,:])
            p_pred = self.rf_model.predict(p.T)
            price = self.prep['time_series'].iloc[indx]
            results.loc[i,'price'] = price
            prediction = (p_pred +1)*price
            results.loc[i,'expected price'] = prediction
